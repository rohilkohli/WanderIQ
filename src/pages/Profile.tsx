...
const STYLE_LABELS: Record<TravelStyle, string> = {
  adventure: '🧗 Adventure', cultural: '🏛️ Cultural', wellness: '🧘 Wellness',
  food: '🍜 Food & Drink', beach: '🏖️ Beach', wildlife: '🦁 Wildlife',
  city: '🏙️ City Break', offbeat: '🗺️ Off-beaten-path',
};
  ...
  return (
    <div style={{ padding: 'var(--space-8)', maxWidth: 900, margin: '0 auto' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-6)', marginBottom: 'var(--space-8)', animation: 'fadeInUp 300ms ease-out' }}>
        <img
          src={user?.photoURL ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.displayName ?? 'Traveller')}&size=100&background=1B4332&color=fff`}
          alt={`${user?.displayName ?? 'Your'} profile photo`}
          width={100}
          height={100}
          style={{ borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--color-accent)' }}
        />
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', marginBottom: 'var(--space-1)' }}>
            {user?.displayName ?? 'Traveller'}
          </h1>
          <p style={{ color: 'var(--color-text-muted)', marginBottom: 'var(--space-3)' }}>
            {user?.email ?? 'guest@wanderiq.app'}
          </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-6)' }}>
        ...
        <section aria-labelledby="travel-prefs-heading" className="card" style={{ padding: 'var(--space-6)', animation: 'fadeInUp 300ms ease-out 60ms both' }}>
          ...
          <div style={{ marginBottom: 'var(--space-5)' }}>
            <h3 style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 'var(--space-3)' }}>
              Budget Tier
            </h3>
            <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
              {(['economy', 'mid-range', 'luxury'] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => editing && setPreferences({ budgetTier: t })}
                  className={`tag${preferences.budgetTier === t ? ' active' : ''}`}
                  disabled={!editing}
                  aria-pressed={preferences.budgetTier === t}
          ...
          <div style={{ marginBottom: 'var(--space-5)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
              {preferences.travelStyle.map((s) => (
                <span key={s} className="badge badge-accent">{STYLE_LABELS[s]}</span>
              ))}
              {editing && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)', width: '100%', marginTop: 'var(--space-2)' }}>
                  {(Object.keys(STYLE_LABELS) as TravelStyle[]).map((s) => (
                    <button
                      key={s}
                      onClick={() => setPreferences({ travelStyle: toggle(preferences.travelStyle, s) as TravelStyle[] })}
                      className={`tag${preferences.travelStyle.includes(s) ? ' active' : ''}`}
                      aria-pressed={preferences.travelStyle.includes(s)}
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
              {([
                ...
                <button
                  key={g.value}
                  onClick={() => editing && setPreferences({ group: g.value })}
          ...
          <div style={{ marginBottom: 'var(--space-5)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
              {(['none', 'vegetarian', 'vegan', 'halal', 'kosher', 'gluten-free'] as DietaryRestriction[]).map((d) => (
                <button
                  key={d}
                  onClick={() => editing && setPreferences({ dietary: toggle(preferences.dietary, d) as DietaryRestriction[] })}
          ...
          <div style={{ marginBottom: 'var(--space-5)' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
              {([
                ...
                <button
                  key={m.value}
                  onClick={() => editing && setPreferences({ mobility: m.value })}
          ...
          <div>
            ...
            {editing ? (
              <input
                ...
                aria-label="Home city for flight searches"
              />
            ) : (
              <p style={{ fontWeight: 600 }}>{preferences.homeCity ?? 'Not set'}</p>
            )}
          </div>
        </section>
          ...
          {[
            {
              ...
              control: (
                <button
                  ...
                  aria-label={`Dark mode is ${isDark ? 'on' : 'off'}`}
                >
                  <span style={{
                    position:   'absolute',
                ...
                <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                  {(['INR', 'USD'] as const).map((c) => (
                    <button
                      ...
                      className={`btn btn-sm ${preferences.currency === c ? 'btn-primary' : 'btn-ghost'}`}
        ...
        <section aria-labelledby="interests-heading" className="card" style={{ padding: 'var(--space-6)', animation: 'fadeInUp 300ms ease-out 240ms both' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
            {([
              { value: 'history', label: '📜 History' }, { value: 'architecture', label: '🏰 Architecture' },
              { value: 'art', label: '🎨 Art' }, { value: 'nightlife', label: '🎵 Nightlife' },
              { value: 'hiking', label: '🥾 Hiking' }, { value: 'photography', label: '📷 Photography' },
              { value: 'shopping', label: '🛍️ Shopping' }, { value: 'literature', label: '📚 Literature' },
              { value: 'music', label: '🎶 Music' }, { value: 'sports', label: '⚽ Sports' },
            ] as { value: Interest; label: string }[]).map((i) => (
              <button
                key={i.value}
                onClick={() => editing && setPreferences({ interests: toggle(preferences.interests, i.value) as Interest[] })}
