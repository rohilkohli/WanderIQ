  ...
  return (
    <div id="main-content" style={{ padding: 'var(--space-8)', maxWidth: 1100, margin: '0 auto' }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                background:    item.primary ? 'var(--color-accent)' : 'var(--color-surface)',
                border:        `1px solid ${item.primary ? 'transparent' : 'var(--color-border)'}`,
                cursor:        'pointer',
            ...
            >
              ...
              <span style={{ fontWeight: 700, color: item.primary ? 'white' : 'var(--color-text-primary)', fontSize: '0.9375rem' }}>
                {item.label}
              </span>
              <span style={{ fontSize: '0.8125rem', color: item.primary ? 'rgba(255,255,255,0.75)' : 'var(--color-text-muted)' }}>
                {item.desc}
              </span>
            </button>
          ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 'var(--space-8)', alignItems: 'start' }}>
        ...
        <div>
          {/* Tabs */}
          <div style={{ display: 'flex', gap: 'var(--space-1)', marginBottom: 'var(--space-6)', borderBottom: '1px solid var(--color-border)', paddingBottom: 'var(--space-1)' }}>
          ...
          <section aria-label={`${tab} trips`}>
            <ul role="list" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', listStyle: 'none' }}>
              {(tab === 'upcoming' ? UPCOMING_TRIPS : PAST_TRIPS).map((trip) => (
                <li key={trip.id} role="listitem">
                  <article
                    className="card"
                    style={{ padding: 'var(--space-5)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)', cursor: 'pointer' }}
                    onClick={() => navigate(`/planner/${trip.id}`)}
                    onKeyDown={(e) => e.key === 'Enter' && navigate(`/planner/${trip.id}`)}
                    tabIndex={0}
                    aria-label={`${trip.destination} trip, ${trip.dates}`}
                  >
                    <div style={{
                      width:          64, height: 64,
                      borderRadius:   'var(--radius-lg)',
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-1)' }}>
                        ...
                        {'status' in trip && (
                          <span className={`badge ${'status' in trip && (trip as {status:string}).status === 'active' ? 'badge-accent' : 'badge-muted'}`}>
                            {(trip as {status:string}).status}
                          </span>
                        )}
                      </div>
                      <p style={{ color: 'var(--color-text-muted)', fontSize: '0.875rem' }}>{trip.dates} · {trip.days} days</p>
                    </div>
                    {'match' in trip && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
          ...
          <section aria-labelledby="wishlist-heading" className="card" style={{ padding: 'var(--space-5)' }}>
            ...
            <ul role="list" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', listStyle: 'none' }}>
              {WISHLIST.map((w) => (
                <li key={w.name} role="listitem" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                    <span aria-hidden="true">{w.emoji}</span>
                    <span style={{ fontWeight: 500 }}>{w.name}</span>
                  </div>
                  <span className="badge badge-muted" style={{ fontSize: '0.7rem' }}>{w.tag}</span>
                </li>
              ))}
            </ul>
              ...
              {BADGES.map((b) => (
                <div
                  ...
                  style={{
                    ...
                    border:     `1px solid ${b.earned ? 'var(--color-accent)' : 'var(--color-border)'}`,
                    background:  b.earned ? 'var(--color-accent-light)' : 'var(--color-surface-alt)',
                    opacity:    b.earned ? 1 : 0.6,
                    textAlign:  'center',
                  }}
                  role="img"
                  aria-label={`${b.label}: ${b.earned ? 'earned' : 'not yet earned'}. ${b.desc}`}
                >
                  <div style={{ fontSize: '1.5rem', marginBottom: 'var(--space-1)' }}>{b.icon}</div>
                  <div style={{ fontSize: '0.6875rem', fontWeight: 700, color: b.earned ? 'var(--color-accent)' : 'var(--color-text-muted)', lineHeight: 1.3 }}>
                    {b.label}
                  </div>
                </div>
              ))}
