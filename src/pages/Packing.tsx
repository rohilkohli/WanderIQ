...
const Packing: React.FC = () => {
  ...
  const addCustomItem = () => {
    ...
    setCategories((prev) =>
      prev.map((c) =>
        c.name === newItemCat ? { ...c, items: [...c.items, newItem] } : c
      )
    );
    setNewItemText('');
    ...
    <div style={{ padding: 'var(--space-8)', maxWidth: 900, margin: '0 auto' }}>
      ...
      <header style={{ marginBottom: 'var(--space-8)', animation: 'fadeInUp 300ms ease-out' }}>
        ...
        <p style={{ color: 'var(--color-text-muted)', marginBottom: 'var(--space-4)' }}>
          AI-generated for your Goa trip · 7 days · Beach & Cultural
        </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          <div className="progress-bar" style={{ flex: 1, height: 10 }}>
            <div
              ...
              role="progressbar"
              aria-valuenow={checkedItems}
              aria-valuemin={0}
              aria-valuemax={totalItems}
              aria-label={`${checkedItems} of ${totalItems} items packed`}
            />
          </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-6)', animation: 'fadeInUp 300ms ease-out 60ms both' }}>
        ...
        >
          {loading ? (
            <span style={{ display: 'inline-block', width: 16, height: 16, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
          ) : '✨'}
          Regenerate with Gemini AI
        </button>
        <button
          onClick={() => setCategories((prev) => prev.map((c) => ({ ...c, items: c.items.map((i) => ({ ...i, checked: false })) })))}
                  ...
                  <li key={item.id} role="listitem">
                    <label
                      style={{
                        ...
                        background:  item.checked ? 'var(--color-accent-light)' : 'transparent',
                    ...
                    >
                      <input
                        ...
                        onChange={() => toggleItem(cat.name, item.id)}
                      ...
                      {item.isCustom && (
                        <span className="badge badge-highlight" style={{ fontSize: '0.6875rem' }}>Custom</span>
                      )}
                    </label>
                  </li>
                ))}
      ...
      <section aria-labelledby="add-item-heading" className="card" style={{ padding: 'var(--space-5)', marginTop: 'var(--space-6)', animation: 'fadeInUp 300ms ease-out 240ms both' }}>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
          <div style={{ flex: 2, minWidth: 200 }}>
            <label htmlFor="new-item-input" className="sr-only">New item name</label>
            <input
              id="new-item-input"
              type="text"
              value={newItemText}
              ...
              placeholder="e.g. Travel pillow, Voltage converter…"
              className="input"
            />
          </div>
          <div style={{ flex: 1, minWidth: 150 }}>
            <label htmlFor="item-category" className="sr-only">Item category</label>
            <select
              id="item-category"
              value={newItemCat}
