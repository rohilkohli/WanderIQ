...
const Packing: React.FC = () => {
  ...
  // Auto-generate once when destination is available without mutating state synchronously in effect
  React.useEffect(() => {
    if (!activeItinerary?.destination || categories.length > 0 || loading || autoTriggerRef.current) return;
  ...
  }, [activeItinerary?.destination, categories.length, loading]);
