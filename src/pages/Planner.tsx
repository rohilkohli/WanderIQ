...
const Planner: React.FC = () => {
  ...
  const handleAutoFill = async (dayId: string, slot: TimeSlot) => {
    ...
    const demoFill: ActivityCard = {
      id: generateId(), name: slot === 'morning' ? 'Sunrise Yoga' : slot === 'afternoon' ? 'Local Market' : 'Beach Dinner',
      category: slot === 'morning' ? 'wellness' : slot === 'afternoon' ? 'experience' : 'restaurant',
      description: 'AI-suggested activity.', address: 'Goa, India', location: { lat: 15.2993, lng: 74.1240 },
      duration: 90, estimatedCost: slot === 'morning' ? 500 : slot === 'afternoon' ? 200 : 800, source: 'ai', tags: ['AI Suggestion'],
    };
    ...
    <div style={{ height: 'calc(100vh - var(--topbar-height))', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)', padding: 'var(--space-3) var(--space-6)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)', flexShrink: 0 }}>
        <div>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)' }}>Jun 15 – Jun 22 · {days.length} days · {formatCurrency(totalSpend)} / {formatCurrency(totalBudget)}</p>
        </div>
        <div style={{ flex: 1 }} />
        ...
        <button onClick={() => setMapVisible(!mapVisible)} className={`btn btn-sm ${mapVisible ? 'btn-primary' : 'btn-ghost'}`}>🗺️ {mapVisible ? 'Hide Map' : 'Show Map'}</button>
      </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: mapVisible ? '1fr 420px' : '1fr', overflow: 'hidden' }}>
        <div style={{ overflowY: 'auto', padding: 'var(--space-6)' }}>
          <div style={{ display: 'flex', gap: 'var(--space-2)', marginBottom: 'var(--space-6)', overflowX: 'auto', paddingBottom: 'var(--space-2)' }}>
            ...
            <button onClick={handleAddDay} style={{ padding: 'var(--space-3) var(--space-4)', borderRadius: 'var(--radius-md)', border: '2px dashed var(--color-border)', background: 'transparent', color: 'var(--color-text-muted)', cursor: 'pointer', fontSize: '0.875rem', fontFamily: 'var(--font-body)', minHeight: 44 }}>+ Add Day</button>
          </div>
