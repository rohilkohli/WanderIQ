  ...
  return (
    <div style={{ padding: 'var(--space-8)', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <section aria-label="Budget summary" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 'var(--space-4)', marginBottom: 'var(--space-8)', animation: 'fadeInUp 300ms ease-out 60ms both' }}>
        {[
          ...
          <div key={card.label} className="card" style={{ padding: 'var(--space-5)' }}>
            <span style={{ fontSize: '1.5rem', display: 'block', marginBottom: 'var(--space-2)' }} aria-hidden="true">{card.icon}</span>
            <div style={{ fontWeight: 700, fontSize: '1.5rem', color: card.color, marginBottom: 'var(--space-1)', fontFamily: 'var(--font-mono)' }}>
              {card.value}
            </div>
      ...
      <section aria-label="Budget progress" className="card" style={{ padding: 'var(--space-6)', marginBottom: 'var(--space-8)', animation: 'fadeInUp 300ms ease-out 120ms both' }}>
        ...
        <div className="progress-bar" style={{ height: 12, borderRadius: 'var(--radius-full)' }}>
          <div
            ...
            style={{ width: `${pct}%`, background: healthColors[health], borderRadius: 'var(--radius-full)' }}
            role="progressbar"
            aria-valuenow={spent}
            aria-valuemin={0}
            aria-valuemax={total}
      ...
      <section aria-labelledby="charts-heading" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-8)', marginBottom: 'var(--space-8)', animation: 'fadeInUp 300ms ease-out 180ms both' }}>
        ...
        <div className="card" style={{ padding: 'var(--space-6)' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.125rem', marginBottom: 'var(--space-4)' }}>
            Budget vs Estimated
          </h2>
          <ResponsiveContainer width="100%" height={240}>
            ...
            return (
              <div key={key}>
                ...
                <div className="progress-bar">
                  <div
                    className="progress-bar-fill"
                    style={{ width: `${pctOfTotal}%`, background: CATEGORY_COLORS[key as keyof typeof CATEGORY_COLORS] }}
                    role="progressbar"
                    aria-valuenow={value}
                    aria-valuemin={0}
                    aria-valuemax={total}
                    aria-label={`${key}: ${formatCurrency(value)}`}
                  />
                </div>
              </div>
            );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', animation: 'fadeInUp 300ms ease-out' }}>
            {AI_SUGGESTIONS.map((s) => (
              <div
                key={s.title}
                className="card"
                style={{ padding: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)', opacity: appliedSuggestions.includes(s.title) ? 0.6 : 1 }}
              >
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontWeight: 700, color: 'var(--color-success)', fontSize: '0.9375rem' }}>
                    -{formatCurrency(s.savings)}
                  </div>
                  <button
                    onClick={() => applySuggestion(s.title, s.savings)}
                    disabled={appliedSuggestions.includes(s.title)}
