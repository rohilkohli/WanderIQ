  ...
  return (
    <div className="app-shell" style={{ minHeight: "100vh" }}>
      ...
      <header className="app-topbar" role="banner" aria-label="WanderIQ top navigation">
        ...
        {/* Action buttons */}
        <button onClick={() => setShowChat(true)} className="btn btn-primary btn-sm" aria-label="Open AI travel assistant" style={{ gap: "var(--space-2)" }}>
          <span aria-hidden="true">✨</span>
          AI Assistant
        </button>

        <button onClick={toggleDark} className="btn btn-ghost btn-sm" aria-label={isDark ? "Switch to light mode" : "Switch to dark mode"} style={{ fontSize: "1.25rem", padding: "var(--space-2)" }}>
          {isDark ? "☀️" : "🌙"}
        </button>

        {user ? (
          <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
            <img src={user.photoURL ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName ?? "U")}&background=1B4332&color=fff`} alt={`${user.displayName ?? "User"} avatar`} width={36} height={36} style={{ borderRadius: "50%", objectFit: "cover", border: "2px solid var(--color-border)" }} />
      ...
      <aside className="app-sidebar" aria-label="Primary navigation">
        ...
        <div style={{ padding: "var(--space-4)", borderTop: "1px solid var(--color-border)" }}>
          <button className="btn btn-highlight" style={{ width: "100%", borderRadius: "var(--radius-md)" }} onClick={() => navigate("/planner")}>
            <span aria-hidden="true">+</span>
            New Trip
          </button>
        </div>
