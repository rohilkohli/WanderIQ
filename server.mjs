// ============================================================
// WanderIQ — Express API Server (Cloud Run)
...
app.post('/api/discover', async (req, res) => {
  ...
  const locationHint = userLat && userLng
    ? `User is currently at ${Number(userLat).toFixed(2)}°N, ${Number(userLng).toFixed(2)}°E. Prioritize reachable Indian destinations.`
...
Return a JSON array of exactly 5 travel destinations. Each object:
{
  "id": "unique-slug",
  "name": "Name",
  "country": "India",
...
IMPORTANT: heroImageUrl must be a real Unsplash URL for that destination.
...
app.post('/api/autofill', async (req, res) => {
  ...
  if (!destination || !slot) return res.status(400).json({ error: 'destination and slot required' });
  if (!GEMINI_API_KEY) return res.status(503).json({ error: 'AI service not configured' });
...
// ── POST /api/itinerary-generate — Full Trip Builder ─────────
...
Build a ${days}-day itinerary for ${destination}.
Total budget: ₹${budget ?? 40000}
Preferences: ${JSON.stringify(preferences ?? {})}

Return a JSON array of ${days} day objects:
[{
  "dayNumber": 1,
  "morning": [{ "name": "", "category": "attraction|restaurant|nature|experience|wellness|shopping", "description": "", "address": "", "location": { "lat": 0, "lng": 0 }, "duration": 90, "estimatedCost": 500, "tags": [], "rating": 4.2 }],
  "afternoon": [...same structure...],
  "evening": [...same structure...]
}]

Each day should have 1-2 activities per slot. Stay within total budget.
